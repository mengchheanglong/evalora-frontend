...
| `/admin` | Platform **admin** only | Admin Hub: usage & cost overview with live system health, organizations (plan changes, suspend/reactivate), and a users directory (deactivate/reactivate, change role). The sidebar link appears only for admins; other roles are redirected to `/dashboard`, and the backend rejects them with `403` regardless. Tabs persist in the URL hash (`#organizations`, `#users`). | `GET /api/admin/overview`, `GET /api/admin/organizations`, `PATCH /api/admin/organizations/:id/{status,plan}`, `GET /api/admin/users`, `PATCH /api/admin/users/:id/{status,role}` |
