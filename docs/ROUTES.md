...
| `/admin` | Platform **admin** only | Platform console, Usage & Cost: a "needs attention" strip linking into filtered lists, four KPI cards with 30-day comparison and sparkline, a 30-day activity chart (sessions started/completed, new accounts, new workspaces, billable AI turns), a status-page style system health panel that re-measures every 30 s while the tab is visible (with a database-latency trend), AI cost with month-end projection, plan mix, and recently joined workspaces. Renders in its own `AdminShell` with its own sidebar and a Ctrl/⌘+K search palette over workspaces and people; the workspace sidebar never links here. Admins land here after sign-in; anonymous visitors go to `/login`, every other role to `/dashboard`, and the backend rejects them with `403` regardless. Legacy `/admin#organizations` and `/admin#users` links forward to the routes below. | `GET /api/admin/overview`, `GET /api/admin/organizations` |
