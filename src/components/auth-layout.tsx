    ...
    <main className="grid min-h-screen bg-white text-neutral-950 lg:grid-cols-[46%_54%]">
      <aside className="auth-visual-panel relative hidden overflow-hidden border-r border-[#dce5e9] bg-[#eff7f9] lg:sticky lg:top-0 lg:flex lg:h-screen lg:self-start lg:flex-col lg:justify-between lg:p-10 xl:p-14">
        ...
        <div className="relative w-full max-w-[560px] py-12">
          {headline ? <div className="max-w-[560px] text-5xl font-black leading-[1.08] text-neutral-950">{headline}</div> : null}
          <p className="mt-5 max-w-[520px] text-base leading-6 text-neutral-600">{lead}</p>
          <div className="mt-9 space-y-4">
            {trustItems.map((item) => (
              <div className="auth-trust-row flex items-center gap-4 rounded-[10px] p-2" key={item.title}>
                ...
                <div>
                  <h2 className="text-sm font-bold text-neutral-950">{item.title}</h2>
                  <p className="mt-1 max-w-[430px] text-xs leading-5 text-neutral-600">{item.body}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
        <p className="relative text-center text-xs text-neutral-500">&copy; 2026 Evalora. All rights reserved.</p>
      </aside>
