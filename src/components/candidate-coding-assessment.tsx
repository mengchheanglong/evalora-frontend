...
export function CandidateCodingAssessment({ accessCode, onBack, onContinue, locked = false }: CandidateCodingAssessmentProps) {
  ...
  return (
    <section className="overflow-hidden rounded-[8px] border border-[var(--theme-border)] bg-[var(--theme-panel)] shadow-[var(--shadow-card)]">
      ...
      {error ? <div className="status-alert status-alert--error rounded-none border-x-0 border-t-0 px-5 py-3 text-xs" role="alert">{error}</div> : null}
      ...
      <div className="grid min-h-[620px] lg:grid-cols-[220px_minmax(0,1fr)]">
        <nav aria-label="Coding challenges" className="border-b border-[var(--theme-border)] bg-[var(--theme-panel-soft)] p-3 lg:border-b-0 lg:border-r">
          <p className="px-2 pb-2 text-xs font-bold uppercase text-[var(--theme-faint)]">Challenges</p>
          <div className="grid gap-1 sm:grid-cols-3 lg:grid-cols-1">
            {questions.map((question, index) => {
              const active = index === activeIndex;
              const result = results[question.id];
              return <button className={`flex min-h-11 items-center gap-2.5 rounded-[6px] px-2.5 py-1.5 text-left transition ${active ? "bg-[var(--theme-panel)] shadow-sm ring-1 ring-[var(--theme-border)]" : "hover:bg-[var(--theme-panel)]"}`} key={question.id} onClick={() => { setActiveIndex(index); setTerminal(result ?? null); setError(""); }} type="button"><span className={`flex size-6 shrink-0 items-center justify-center rounded-[5px] text-xs font-black ${result ? "bg-emerald-100 text-emerald-700" : active ? "bg-[var(--theme-active)] text-[var(--theme-active-text)]" : "bg-[var(--theme-panel-soft)] text-[var(--theme-muted)]"}`}>{result ? <Icon name="check" size={11} /> : index + 1}</span><span className="min-w-0"><span className="block truncate text-xs font-bold text-[var(--theme-heading)]">{question.title}</span><span className="block text-xs capitalize text-[var(--theme-muted)]">{result ? `${result.score}% score` : question.difficulty}</span></span></button>;
            })}
          </div>
        </nav>
        ...
        <div className="grid min-w-0 xl:grid-cols-[minmax(260px,0.72fr)_minmax(420px,1.28fr)]">
          <article className="border-b border-[var(--theme-border)] p-5 xl:border-b-0 xl:border-r">
            ...
            <div className="mt-5 rounded-[6px] border border-[var(--theme-border)] bg-[var(--theme-active)] px-3 py-2.5 text-xs text-[var(--theme-active-text)]">Hidden inputs and expected outputs stay private. You receive pass/fail evidence after submission.</div>
          </article>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--theme-border)] bg-[var(--theme-panel)] px-4 py-3 sm:px-5"><p className="text-xs text-[var(--theme-muted)]">Your latest submission for each challenge is included in the reviewer report.</p><button className="button-primary !min-h-9 !text-xs" disabled={!allSubmitted || locked} onClick={onContinue} type="button">Continue assessment <Icon className="-rotate-90" name="chevron" size={12} /></button></footer>
    </section>
  );
}
