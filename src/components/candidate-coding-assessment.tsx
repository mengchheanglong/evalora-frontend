  ...
  return (
    <section className="overflow-hidden rounded-[8px] border border-[var(--theme-border)] bg-[var(--theme-panel)] shadow-[var(--shadow-card)]">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--theme-border)] px-4 py-3 sm:px-5">
        <div><p className="text-[var(--text-micro)] font-bold uppercase text-[var(--color-primary-700)]">Coding assessment</p><h2 className="mt-1 text-[var(--text-body)] font-black text-[var(--theme-heading)]">JavaScript sandbox</h2></div>
        <div className="flex items-center gap-3"><span className="text-[var(--text-micro)] font-semibold text-[var(--theme-muted)]">{completedCount}/{questions.length} submitted</span><div className="h-1.5 w-28 overflow-hidden rounded-full bg-[var(--theme-panel-soft)]"><div className="h-full rounded-full bg-emerald-500 transition-all" style={{ width: `${questions.length ? (completedCount / questions.length) * 100 : 0}%` }} /></div></div>
      </header>
      ...
      <div className="grid min-h-[620px] lg:grid-cols-[220px_minmax(0,1fr)]">
        <nav aria-label="Coding challenges" className="border-b border-[var(--theme-border)] bg-[var(--theme-panel-soft)] p-3 lg:border-b-0 lg:border-r">
          <p className="px-2 pb-2 text-[var(--text-micro)] font-bold uppercase text-[var(--theme-faint)]">Challenges</p>
          <div className="grid gap-1 sm:grid-cols-3 lg:grid-cols-1">
            {questions.map((question, index) => {
              const active = index === activeIndex;
              const result = results[question.id];
              return <button className={`flex min-h-14 items-center gap-3 rounded-[6px] px-3 py-2 text-left transition ${active ? "bg-[var(--theme-panel)] shadow-sm ring-1 ring-[var(--theme-border)]" : "hover:bg-[var(--theme-panel)]"}`} key={question.id} onClick={() => { setActiveIndex(index); setTerminal(result ?? null); setError(""); }} type="button"><span className={`flex size-7 shrink-0 items-center justify-center rounded-[5px] text-[var(--text-micro)] font-black ${result ? "bg-emerald-100 text-emerald-700" : active ? "bg-[var(--theme-active)] text-[var(--theme-active-text)]" : "bg-[var(--theme-panel-soft)] text-[var(--theme-muted)]"}`}>{result ? <Icon name="check" size={13} /> : index + 1}</span><span className="min-w-0"><span className="block truncate text-[var(--text-micro)] font-bold text-[var(--theme-heading)]">{question.title}</span><span className="mt-0.5 block text-[var(--text-micro)] capitalize text-[var(--theme-muted)]">{result ? `${result.score}% score` : question.difficulty}</span></span></button>;
            })}
          </div>
        </nav>
        ...
        <div className="grid min-w-0 xl:grid-cols-[minmax(260px,0.72fr)_minmax(420px,1.28fr)]">
          <article className="border-b border-[var(--theme-border)] p-5 xl:border-b-0 xl:border-r">
            <div className="flex items-center justify-between gap-3"><span className={`rounded-[4px] px-2 py-1 text-[var(--text-micro)] font-bold capitalize ${activeQuestion.difficulty === "easy" ? "bg-emerald-50 text-emerald-700" : activeQuestion.difficulty === "medium" ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}>{activeQuestion.difficulty}</span><span className="text-[var(--text-micro)] font-semibold text-[var(--theme-faint)]">{activeQuestion.testCaseCount} hidden tests</span></div>
            <h3 className="mt-4 text-[var(--text-h3)] font-black text-[var(--theme-heading)]">{activeQuestion.title}</h3>
            <p className="mt-3 text-[var(--text-caption)] leading-[var(--text-caption--line-height)] text-[var(--theme-muted)]">{activeQuestion.description}</p>
            ...
            <div className="mt-6 rounded-[6px] border border-[var(--theme-border)] bg-[var(--theme-active)] p-3 text-[var(--text-micro)] leading-[var(--text-micro--line-height)] text-[var(--theme-active-text)]">Hidden inputs and expected outputs stay private. You receive pass/fail evidence after submission.</div>
          </article>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--theme-border)] bg-[var(--theme-panel)] px-4 py-3 sm:px-5"><p className="text-[var(--text-micro)] text-[var(--theme-muted)]">Your latest submission for each challenge is included in the reviewer report.</p><button className="button-primary" disabled={!allSubmitted || locked} onClick={onContinue} type="button">Continue assessment <Icon className="-rotate-90" name="chevron" size={13} /></button></footer>
    </section>
  );
}

function InfoBlock({ label, children }: { label: string; children: React.ReactNode }) { return <div><p className="mb-2 text-[var(--text-micro)] font-bold uppercase text-[var(--theme-faint)]">{label}</p><div className="overflow-auto rounded-[6px] bg-neutral-950 p-3 font-mono text-[11px] leading-5 text-slate-200">{children}</div></div>; }
