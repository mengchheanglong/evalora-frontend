      ...
      <div className="mx-auto max-w-[1200px] space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-xl font-black tracking-tight text-[var(--theme-heading)]">Edit assessment template</h1>
        ...
        <section className="rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-panel)] p-5 shadow-sm">
          <h2 className="text-sm font-black uppercase tracking-wide text-[var(--theme-muted)]">Template details</h2>
        ...
        <div className="grid gap-4 lg:grid-cols-[220px_1fr]">
          <aside className="h-fit rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-panel)] p-3 shadow-sm lg:sticky lg:top-4">
            <div className="mb-2 flex items-center justify-between px-1">
              <p className="text-xs font-bold uppercase tracking-wide text-[var(--theme-faint)]">Modules</p>
              ...
              {modules.map((module, index) => (
                <li key={module.key}>
                  <button
                    ...
                    type="button"
                  >
                    <span className="text-xs font-bold text-[var(--theme-faint)]">M{index + 1}</span>
                    <span className="mt-0.5 block truncate text-xs font-bold text-[var(--theme-heading)]">{module.title || "Untitled"}</span>
                    <span className="text-xs font-medium text-[var(--theme-muted)]">{module.questions.length} questions</span>
                  </button>
                </li>
              ))}
            </ul>
          ...
          <div className="space-y-4">
            {modules.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-[var(--theme-border-strong)] bg-[var(--theme-panel-soft)] p-8 text-center">
                <p className="text-sm font-bold text-[var(--theme-text)]">No modules yet</p>
                <p className="mt-1 text-xs text-[var(--theme-muted)]">Add a module to start editing questions.</p>
                ...
                <section className="overflow-hidden rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-panel)] shadow-sm" id={`edit-module-${module.key}`} key={module.key}>
                  <header className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--theme-border)] bg-[var(--theme-panel-soft)] px-4 py-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold uppercase tracking-wide text-[var(--theme-faint)]">Module {moduleIndex + 1}</p>
                      <div className="mt-2 grid gap-2 sm:grid-cols-2">
                        ...
                        <textarea
                          className="control min-h-[44px] rounded-xl text-sm leading-5 sm:col-span-2"
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button className="h-9 rounded-lg border border-[var(--color-status-critical)]/40 px-3 text-xs font-bold text-[var(--color-status-critical)] hover:bg-[var(--color-status-critical)]/10" onClick={() => setPendingModuleRemoval(module.key)} type="button">
                        Remove module
                      </button>
                    </div>
                  </header>
                      ...
                      {module.type === "ai_interview" ? (
                        ...
                        <article className="rounded-xl border border-[var(--theme-border)] bg-[var(--theme-panel-soft)] p-4" key={question.key}>
                          ...
                          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                            <p className="text-xs font-black text-[var(--theme-text)]">Question {questionIndex + 1}</p>
                            <div className="flex flex-wrap items-center gap-1">
                              ...
                              <button
                                ...
                                className="ml-1 rounded-lg px-2 py-1 text-xs font-bold text-[var(--theme-muted)] transition-colors hover:bg-[var(--color-status-critical)]/12 hover:text-[var(--color-status-critical)] disabled:pointer-events-none disabled:opacity-40"
        ...
        <div className="sticky bottom-4 z-10 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-panel)] p-3 shadow-lg backdrop-blur">
          <p className="px-2 text-xs font-medium text-[var(--theme-muted)]">
