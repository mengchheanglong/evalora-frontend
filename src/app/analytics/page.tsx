    ...
    <div className="space-y-6">
      ...
      <section>
        <div className="flex flex-col gap-3 rounded-[10px] border border-[var(--theme-border)] bg-[var(--theme-panel)] p-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-base font-extrabold text-[var(--theme-heading)]">Results by assessment</h2>
            <p className="mt-1 text-xs leading-4 text-[var(--theme-muted)]">Candidates are only compared against others who took the same assessment.</p>
          </div>
          <label className="min-w-0 sm:w-[320px]">
            ...
            <select className="control h-10 text-xs" disabled={!usage.length} onChange={(event) => onTemplateChange(event.target.value)} value={selectedTemplateId}>
      ...
      {usage.map((item) => {
        ...
        return (
          <li className="flex items-center gap-4 py-2.5 first:pt-0 last:pb-0" key={item.templateId}>
            <span className="min-w-0 flex-1 truncate text-xs font-semibold text-[var(--theme-text)]" title={item.title}>
              {label}
            </span>
            ...
            <span className="w-28 shrink-0 text-right text-xs tabular-nums text-[var(--theme-muted)]">
